...
-- KOF Andrew: o launcher atualiza este estado imediatamente antes de abrir o
-- jogo. Sem uma confirmacao recente do servidor, o item ONLINE fica oculto.
local koffStatusFile = io.open('save/koff-server-status.txt', 'r')
local koffServerOnline = false
if koffStatusFile ~= nil then
	local koffStatus = koffStatusFile:read('*all') or ''
	koffStatusFile:close()
	koffServerOnline = koffStatus:lower():match('online') ~= nil
end
if not koffServerOnline and motif.title_info ~= nil and motif.title_info.menu ~= nil then
...
-- KOFF launcher: connect both machines and enter the synchronized character
-- selection directly. The host becomes Player 1 and the client Player 2.
local koffNetplay = getCommandLineValue("-koffnetplay")
if koffNetplay ~= nil then
	main.f_waitForPreloads(true)
	local server = ""
	local label = gameOption('Netplay.ListenPort')
	if koffNetplay ~= "host" then
		server = getCommandLineValue("-koffhost") or "127.0.0.1"
		label = server
	end
	if main.f_connect(server, label) then
		if synchronize() then
			main.f_default()
			local selectMode = main.t_itemname['netplayversus']({[1] = {itemname = 'netplayversus'}}, 1)
